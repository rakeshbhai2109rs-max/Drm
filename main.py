...
subprocess.run([
    ...
    "-i", "https://itsgolu-cp-api.vercel.app",
    "-c", "copy",
    "output.mp4"
])
...
api_url = url = "https://itsgolu-cp-api.vercel.app"
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            name1 = links[i][0].replace("(", "[").replace(")", "] ").replace("_", "").replace("\t", "").replace(":", "").replace("/", "").replace("+", "").replace("#", "").replace("|", "").replace("@", "").replace("*", "").replace("[vip]", "⚝").replace(".", "").replace("https", "").replace("http", "").replace("vip", "⚝" ).strip()
            name = f'⚝ {name1[:90]}'
            ...
            if "acecwply" in url:
             ...
             #   url = f"https://scammer-keys.vercel.app/api?url={url}&token={cptoken}&auth=@scammer_botxz1"
                ...
                signed_api = f"https://itsgolu-cp-api.vercel.app/itsgolu?url={url}@ITSGOLU_OFFICIAL&user_id={7735386939}"
